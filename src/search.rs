...
use anyhow::Result;
use libxinux::metadata::search::{get_searcher, SearchQuery, SearchResult};
use owo_colors::{OwoColorize, Stream::Stdout};

pub async fn search(query: &[&str]) -> Result<()> {
    let db = libxinux::metadata::database::database_connection().await?;

    let currprofilepkgs = if is_profile_configured() {
        libxinux::profile::list::list()?
            .into_iter()
    ...
    let currsyspkgs = if is_system_configured() {
        libxinux::nixos::list::list_systempackages(&db)?
            .into_iter()
            .map(|x| x.attr.to_string())
            .collect::<Vec<_>>()
    } else {
    ...
    let currhomepkgs = if is_home_configured() {
        libxinux::homemanager::list::list(&db)?
            .into_iter()
            .map(|x| x.attr.to_string())
            .collect::<Vec<_>>()
    } else {
    ...
    let mut search_result = libxinux::metadata::search::search(
        &SearchQuery {
            query: &query.join(" "),
            limit: 100,
            score_threshold: 0.0,
